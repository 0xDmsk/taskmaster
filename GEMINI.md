...
3.  **Provision**: 
    ...
    *   **Session material** (cookies, tokens, browser state): when the agent needs user-supplied session data, never paste its contents into the mission or arguments (it would leak into the execution request and audit log). Keep the file in a folder inside the engagement directory (e.g. `./session/`, naming a browser login `storage_state.json`), set `session_dir` on `spawn_agent` to the **absolute** path of that folder (resolve `./session` first — the server's working directory differs from yours), and reference it by its container path `/session/...`. It mounts read-only at `/session`; Playwright agents auto-load `/session/storage_state.json`.
