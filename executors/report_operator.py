#!/usr/bin/env python3
"""
Report Operator — document-rendering executor for Taskmaster.

Claims tasks with `action_type == "report_skill"` and runs them through
a `BaseReportSkill` subclass (see `skills/reporting.py`). Mirrors the
playwright operator's polling loop and action-type filter so kali /
playwright / reporting agents can coexist on the same queue without
fighting for tasks they cannot execute.
"""
import importlib
import json
import os
...
EXECUTOR_ID = os.environ.get("EXECUTOR_ID", f"reporting-{socket.gethostname()}")
TARGET_SCOPE = os.environ.get("TARGET_SCOPE")
AGENT_MISSION = os.environ.get("AGENT_MISSION")

SUPPORTED_ACTION_TYPES = {"report_skill"}
...
def execute_action(execution):
    ...
    if action_type == "report_skill":
        return _execute_report_skill(payload)
    return {
        "status": "FAILED",
        "result": json.dumps(
            {
...
def _execute_report_skill(payload):
    """Import and run a BaseReportSkill subclass."""
    skill_name = payload.get("skill")
    skill_args = payload.get("arguments", {})
    target = payload.get("target")

    print(f"[*] Invoking Report Skill: {skill_name} with {skill_args}")

    try:
        # Format: 'reporting.FindingDocxReport'
...
def main_loop():
    print(f"[*] Report Operator started (ID: {EXECUTOR_ID})")
    if TARGET_SCOPE:
    ...
    while True:
        ...
        for task in executions:
            ...
            # Only handle report tasks — leave everything else for other executors
